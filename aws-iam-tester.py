...
# Defaults
DEFAULT_USER_LANDING_ACCOUNT=155223579331
...
@click.option('--user-landing-account', '-u', help='Account id for user landing.', default=DEFAULT_USER_LANDING_ACCOUNT)
...
def test_policy(number_of_runs, dry_run, user_landing_account, write_to_file, debug):
    setup_logger(
        debug=debug,
        )
    ...
    logger.debug("get the test config")
    ...
    for c in config['tests']:
        ...
        expect_failures = (c["expected_result"] == "fail")
